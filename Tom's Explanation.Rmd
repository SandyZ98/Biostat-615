...
#Lets call our build_up_main function
#First, use first pass values. Gp estimate using Gp_rate
output_sim_1 = build_up_main(simulated.dat$time,
                     simulated.dat$CO2,
                     NA,
                     n,
                     Gp,
...
output_sim_2 = build_up_main(simulated.dat$time,
                      simulated.dat$CO2,
                      NA,
                      matrix(1,5,1),
                      c(0,7.128,0,7.290,0),
...
output_sim_3 = build_up_main(simulated.dat$time,
                       simulated.dat$CO2,
                       NA,
                       n,
                       Gp/2.5,
...
output_sim_4 = build_up_main(simulated.dat$time,
                       simulated.dat$CO2,
                       NA,
                       n,
...
output_data_1 = build_up_main(data_1$`Time(dd/mm/yyyy)`,
...
output_data_3 = build_up_main(data_1$`Time(dd/mm/yyyy)`,
                            data_1$`Carbon dioxide(ppm)`,
                            NA,
                            3,
                            Gp,
...
output_data_5 = build_up_main(data_1$`Time(dd/mm/yyyy)`,
                            data_1$`Carbon dioxide(ppm)`,
                            NA,
                            5,
                            Gp,
...
output_data_5$raw_co2_df

output_data_5$interval_avg_df
...
for (row in seq(1, nrow(output_data_5$raw_co2_df))) {
  index_change = seq(output_data_5$raw_co2_df$Index.Start[row], output_data_5$raw_co2_df$Index.Stop[row])
...
for (i in seq(1:dim(raw_relerr)[3])){
 raw_relerr[,,i] = t(apply(raw_relerr[,,i], 1, function(x,truth){(x - truth) / truth}, ventilation_true))
 interval_relerr[,,i] = t(apply(interval_relerr[,,i], 1, function(x,truth){(x - truth) / truth}, ventilation_true))
